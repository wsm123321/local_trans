...
即使通过，本 Pilot 也只支持：在二维、固定图表、单专家、共享候选和付费 source-blind 成对诊断反馈下，**在三个冻结估计器共同 eligible 的最终事件支持集上**，冻结的冲突纠错估计器比冻结的 decision-matched Local Spearman 更接近下一次固定源提名的增量价值。它不证明自然单点反馈可学性、实际区域检索、未知对齐、多源调度、在线预算收益、高维泛化或普遍无负迁移。
